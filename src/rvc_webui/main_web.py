...
load_dotenv(PROJECT_ROOT / "sha256.env")
...
(PROJECT_ROOT / "assets" / "weights").mkdir(parents=True, exist_ok=True)
...
if not config.nocheck:
    from rvc_webui.infer.lib.rvcmd import check_all_assets, download_all_assets

    if not check_all_assets(update=config.update):
        if config.update:
            download_all_assets(tmpdir=str(tmp))
            if not check_all_assets(update=config.update):
...
def preprocess_dataset(trainset_dir, exp_dir, sr, n_p):
    sr = sr_dict[sr]
    exp_path = PROJECT_ROOT / "logs" / exp_dir
    os.makedirs(exp_path, exist_ok=True)
    log_file_path = exp_path / "preprocess.log"
    f = open(log_file_path, "w")
    f.close()
    cmd = '"%s" -m rvc_webui.train.preprocess "%s" %s %s "%s" %s %.1f' % (
        config.python_cmd,
        trainset_dir,
        sr,
        n_p,
    ...
    p = Popen(cmd, shell=True, cwd=str(PROJECT_ROOT))
...
def extract_f0_feature(n_p, f0method, if_f0, exp_dir, version19):
    exp_path = PROJECT_ROOT / "logs" / exp_dir
    os.makedirs(exp_path, exist_ok=True)
    f = open(exp_path / "extract_f0_feature.log", "w")
    f.close()
    if if_f0:
        if f0method != "rmvpe_gpu":
            cmd = (
                '"%s" -m rvc_webui.train.extract_f0_print "%s/logs/%s" %s %s "%s" %s'
                % (
                    config.python_cmd,
                    str(PROJECT_ROOT),
                    exp_dir,
    ...
    for idx, n_g in enumerate(gpus):
        cmd = (
            '"%s" -m rvc_webui.train.extract_feature_print %s %s %s %s "%s/logs/%s" %s %s'
            % (
                config.python_cmd,
                config.device,
                leng,
...
def click_train(
    ...
    # 生成filelist
    exp_dir = "%s/logs/%s" % (str(PROJECT_ROOT), exp_dir1)
    ...
    if if_f0_3:
        ...
        for _ in range(2):
            opt.append(
                ...
                % (str(PROJECT_ROOT), sr2, str(PROJECT_ROOT), fea_dim, spk_id5)
            )
    shuffle(opt)
    ...
    if not pathlib.Path(config_save_path).exists():
        with open(config_save_path, "w", encoding="utf-8") as f:
            ...
            f.write("\n")
    cmd = (
        '"%s" -m rvc_webui.train.train -e "%s" -sr %s -f0 %s -bs %s -te %s -se %s %s %s -l %s -c %s -sw %s -v %s -a "%s"'
        % (
            config.python_cmd,
            exp_dir1,
            sr2,
